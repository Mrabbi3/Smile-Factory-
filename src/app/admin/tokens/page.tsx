...
export default function TokensPage() {
  ...
  const loadData = async () => {
    ...
    setTodayStats({
      total: todayTx.reduce((s, t) => s + Number(t.amount_paid), 0),
      cash: todayTx.filter(t => t.payment_type === 'cash').reduce((s, t) => s + Number(t.amount_paid), 0),
      card: todayTx.filter(t => t.payment_type === 'card').reduce((s, t) => s + Number(t.amount_paid), 0),
      tokens: todayTx.reduce((s, t) => s + t.tokens_given, 0),
            ...
            <Table>
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((tx) => (
                    <TableRow key={tx.id}>
                      <TableCell>{format(new Date(tx.created_at), 'MMM dd, yyyy h:mm a')}</TableCell>
